...
*/
						...
						$query
							...
							->where($db->quoteName('sensor_type') . ' LIKE ' . $db->quote($sensorType))
							->where($db->quoteName('sensor_id') . ' LIKE ' . $db->quote($sensorId))
							->where($db->quoteName('created') . ' BETWEEN ' . $db->quote('2020-10-03 '.substr($dates[$i], 0, 2) . ':00:00') . ' AND ' . $db->quote('2020-10-03 '.substr($dates[$i], 0, 2) . ':59:59'));
						...
						$rawData = $db->loadRow();
				        $tempData[] = $rawData[0];
				        $dateData[] = "'" . substr($dates[$i], 0, 2) . ':00' . "'";
